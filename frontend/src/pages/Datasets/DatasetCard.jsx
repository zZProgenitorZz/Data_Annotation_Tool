...
const Row = ({
  ...
  if (canEditThisRow) {
    ...
    if (name === "assignedTo") {
      const rawAssigned = Array.isArray(value) ? value : value ? [value] : [];
        ...
        <div className="grid grid-cols-[120px_163px] gap-x-[8px] mb-[6px] items-center">
          ...
          <div className="mt-2 space-y-2">
            {assignedInfo.map((item) => {
              const userOption = options.find((opt) => opt.value === item.id);
  ...
  if (name === "description") {
    return (
      <div className="mt-[12px]">
        <div className="font-[600] text-[16px] text-[#000000] mb-[2px]">
          {label}:
        </div>

        <p
          className="text-[14px] text-[#333333] border border-[#d1d5db] p-[6px] rounded-[6px] bg-[#ffffff] h-[60px] overflow-y-auto"
          style={{ 
            whiteSpace: "pre-wrap",
            overflowX: "hidden",
