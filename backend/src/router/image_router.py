...
async def soft_delete_images(dataset_id: str, image_id: list[str] | None = None ):
    try:
        return await image_service.soft_delete_images(image_id, dataset_id, current_user = None)
    except NotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))

#restore image
@router.put("/{dataset_id}/restore", response_model=int)
async def restore_image(dataset_id: str, image_id: list[str] | None = None ):
    try:
        return await image_service.restore_images(image_id, dataset_id, current_user= None)
    except NotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))

# hard delete image
@router.delete("/{image_id}/hard", response_model=bool)
async def hard_delete_image(image_id: str):
    try:
        return await image_service.hard_delete_image(image_id)
    except NotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))

# hard delete images (bulk)
@router.delete("/dataset/{dataset_id}/hard", response_model=int)
async def hard_delete_dataset_images(dataset_id: str):

    try:
        return await image_service.hard_delete_dataset_images(dataset_id)
    except NotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
